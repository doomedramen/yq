...
function timeTween(submit, start) {
  ...
  var exact = duration.get('days') + ':' + duration.get("hours") + ":" + duration.get("minutes") + ":" + duration.get("seconds");
...
function parseSacct(str) {
  ...
  if (splitted.length == 4) {
    ...
    return {
      ...
      Partition: cleanedList[3],
      Account: cleanedList[4],
      ReqMem: parseMem(cleanedList[5]),
      NNodes: cleanedList[6]
    };

  } else {
    //ERROR
    console.error('SPLITTED LENGTH != 3');
  }
